...
<span class="marq_a_text"><i class="fa fa-circle"></i> <i>These are free services with an option to make courtesy donation. </i></span>					                                                
    ...
    <div class="main-background">
        ...
        <div class="thirdelement">
            <a href="#"><button class="helpbutton">Blood Help</button></a>
            <a href="index2.php"><button class="helpbutton-active">Missing Help</button></a>
            <a href="#"><button class="helpbutton">Travel Help</button></a>
        </div>
        <br><br><br>
        <div class="firstelement">
            <strong>MISSING HELP</strong>
        </div>
        <br>
        <table class="table table-bordered" class="fourthelement" >
        <tbody>
            <tr>
                <th colspan="7">Missing Details</th>
        <th colspan="2">Contact Details</th>
            ...
            <?php 
             $query = "Select * from vtbl_msnhlp where ActiveStat = 1 order by id DESC";
             $result = mysqli_query($con, $query)or die($mysqli_error($con));
             while ($row = mysqli_fetch_array($result) ){
            ?>
            <tr>
                <td><?php echo $row['Category'] ?></td>
                <td><?php echo $row['MsnName'] ?></td>
                <td><?php echo $row['MsnDate'] ?></td>
                <td><?php echo $row['State'] ?></td>
                <td><?php echo $row['City'] ?></td>
                <td><?php echo $row['MArea'] ?></td>
                <td><?php echo $row['Specifications'] ?></td>
                <td><?php echo $row['ContFName'] ?></td>
